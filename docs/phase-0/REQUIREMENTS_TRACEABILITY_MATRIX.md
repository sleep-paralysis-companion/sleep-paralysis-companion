...
| `P1-ONB-005` | Collect only essential setup information with an approved purpose for every field. | `BLOCKED` | `S-EXEC` Phase 0 and §9.3 | Pending | Product-approved field list with purpose, sensitivity, storage, retention, export, and deletion mapping. |
...
| `P1-ONB-009` | Reconcile the exact onboarding questions and branch logic before implementation. | `BLOCKED` | `S-EXEC` Phase 0; `S-TECH` “M2 Onboarding And Personalization” | Pending | Signed onboarding truth table linked to approved fields, copy, data purposes, and expected destination state. |
...
| `P1-SET-007` | Keep the alarm and required utility routes free; require verified premium for every other feature; offer eligible customers one StoreKit three-day introductory trial in the monthly/annual subscription group. | `OWNER APPROVED` | `D0-011`; `D0-012`; `D0-018`; `COM-P1-001` | Pending | StoreKit tests cover eligible/ineligible trial presentation, conversion, expiry, offline signed expiration, alarm continuity, and noncoercive paywall behavior. |
| `P1-SET-008` | Use StoreKit 2 verified transactions for monthly USD 8.99, annual USD 59.99, lifetime USD 149.99, and 16-day paid-renewal grace; keep restoration, management, privacy/legal/support, export/deletion, and account deletion accessible without entitlement. | `OWNER APPROVED` | `S-APPLE-ARG` §3.1; `D0-011`; `D0-018`; `D0-019` | Pending | App Store Connect, Sandbox, and TestFlight evidence covers purchase, pending, cancel, Ask to Buy, trial, plan change, renewal, retry, grace, lifetime, refund, revoke, restore, reinstall, multiple devices, and absence of Family Sharing. |
...
| `P1-AUTH-002` | Offer Sign in with Apple, Sign in with Google, and email account creation/sign-in for optional synchronization. | `ADOPTED` | `S-PRD-P1-COPY`; `D0-020` | Pending | Approved provider configuration and UI tests cover success, cancellation, provider error, offline state, and usable guest fallback for all three methods. |
| `P1-AUTH-003` | Define deterministic linking/collision behavior when Apple, Google, or email identifies an existing account; never merge identities or data silently. | `BLOCKED` | `D0-010`; `D0-020` | Pending | Security-approved identity table and integration tests cover same-email/different-provider, hidden Apple email, already-linked provider, reauthentication, replay, and cross-account denial. |
| `P1-AUTH-004` | The email method must have approved verification, recovery, reauthentication, rate limiting, enumeration resistance, and abuse controls. | `BLOCKED` | `D0-020`; `S-IOS` §4 | Pending | Threat review and integration tests cover unverified, expired link/code, reset/recovery, resend throttling, enumeration, brute force, token expiry, and account deletion. |
...
| `P1-SYN-002` | Define guest ownership and an atomic guest-to-account transition. | `BLOCKED` | `S-EXEC` §§6, Phase 1B | N/A | Approved transition contract and rollback tests for duplicates, partial failure, existing account data, and retry. |
...
| `P1-SYN-004` | Use a documented deterministic conflict rule per entity; no undocumented last-write-wins. | `BLOCKED` | `S-EXEC` §§5, Phase 1B; `S-SWIFT` §16 | N/A | Approved entity conflict table plus deterministic multi-device/retry tests. |
...
| `P1-SYN-007` | Define deterministic sign-out, token-expiry, reinstall, account-conversion, and account-deletion outcomes. | `BLOCKED` | `S-EXEC` Phase 1B | Pending | Approved lifecycle table plus end-to-end local/Keychain/remote reconciliation tests. |
