...
| `P1-SYN-002` | Define guest ownership and an atomic guest-to-account transition. | `BASELINE` | `DATA-P1-001` §§5.2–5.4; `SPEC-P1-001` §§4, 6.6 | Phase 1B hosted evidence pending | Transition contract and rollback tests cover duplicates, partial failure, existing account data, wrong account, and retry. |
...
| `P1-SYN-004` | Use a documented deterministic conflict rule per entity; no undocumented last-write-wins. | `BASELINE` | `DATA-P1-001` §6; `SPEC-P1-001` §11 | Phase 1B hosted evidence pending | Canonical entity conflict table plus deterministic conflict, retry, replay, and tombstone tests. |
...
| `P1-SYN-007` | Define deterministic sign-out, token-expiry, reinstall, account-conversion, and account-deletion outcomes. | `BASELINE` | `DATA-P1-001` §§5, 7, 8; `SPEC-P1-001` §§6.6, 6.8, 7 | Phase 1B hosted evidence pending | Canonical lifecycle tables plus local/Keychain/isolated-remote reconciliation tests; physical and provider-console evidence remains external. |
