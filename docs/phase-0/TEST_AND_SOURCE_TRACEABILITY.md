...
| `T-SUITE-SET-001` | `P1-SET-001`–`008` | All settings states, external permission changes, export reconciliation, local/account deletion, policy URLs, trial eligibility/conversion, lifetime, RevenueCat/StoreKit lifecycle, no-grace immediate cutoff, and known-expiration reminder |
| `T-SUITE-AUTH-001` | `P1-AUTH-001`–`005` | Guest fallback, Apple/Google success and cancellation, offline/provider errors, excluded email/password/passwordless/phone/OTP methods, identity collision/linking, reauthentication, token expiry/revocation, sign-out, account deletion |
