...
| `D0-004` | Guest/account boundary | Guest/local-first; optional account only when person chooses sync; StoreKit is independent | `ADOPTED` | Lifecycle/security approval and tests |
...
| `D0-011` | Paid feature boundary | One logical `premium_access`; alarm plus required utilities are always free; every other feature is premium | `APPROVED BY SATYAM SHREE — 25 JULY 2026` | StoreKit Sandbox and stressed-paywall evidence |
...
| `D0-018` | StoreKit products | Monthly USD 8.99 and annual USD 59.99 auto-renewables in one group; lifetime USD 149.99 non-consumable; Family Sharing off; 16-day paid-to-paid grace | `APPROVED BY SATYAM SHREE — 25 JULY 2026` | App Store Connect product IDs/localizations and Sandbox evidence |
| `D0-019` | Commercial authority/offline | StoreKit only; no global UTC promotion or custom offline grace; verified signed trial/subscription/lifetime state controls access | `APPROVED BY SATYAM SHREE — 25 JULY 2026` | Offline/expiry/refund/revoke physical and Sandbox evidence |
| `D0-020` | Authentication method | Optional Supabase sync accounts offer Apple, Google, and passwordless email one-time code; guest/local-first remains available; linking requires explicit reauthentication | `APPROVED DIRECTION — 25 JULY 2026` | Callback, abuse, collision, deletion, and RLS test evidence |
...
| `R0-008` | Polar/custom/external payment unlock | StoreKit controls in-app digital premium access |
...
|---|---|---|---|
| `B0-004` | Commercial decisions are approved, but App Store Connect products and StoreKit Sandbox evidence do not exist | iOS + QA + Release | Configure monthly/annual/lifetime products and trial/grace settings; capture eligibility, conversion, restore, expiry, refund, revoke, lifetime, and offline evidence |
| `B0-006` | Supabase direction and lifecycle defaults are approved, but no migrations, RLS/storage policies, provider callbacks, or positive/negative isolation/deletion tests exist | Backend + Security | Produce versioned design/policies and owner-positive, anonymous/other-user denial, forged-owner, expired/revoked-session, over-posted-field, and deletion/tombstone tests |
...
| `B0-008` | AlarmKit, manual surface, notification fallback, audio, lock, restart, time, and accessibility lack physical proof; an iPhone can be arranged but no local Mac is available | iOS + QA/A11y + Design/Privacy | Compile/sign/upload with an approved hosted macOS workflow, distribute by internal TestFlight, run `FEAS-P1-001` on the physical matrix, and use a borrowed/remote interactive Mac only if CI diagnostics are insufficient |
