...
| `D0-030` | Existing Supabase waitlist | `public.waitlist` serves a live website. Preserve existing submissions and availability; verify or update the website to a write-only response before removing anonymous reads and restricting grants through `SUPA-P0-002`. | `OWNER CONFIRMED — 28 JULY 2026` | Website URL/source revision, isolated migration tests, Backend/Security approval, and action-time live-change authorization |
...
| `B0-006` | `SUPA-P0-001` confirms the selected project but found no recorded migrations/Auth identities/Phase 1 schema; the live-website `public.waitlist` anonymously exposes email rows, permits unrestricted inserts, and has broad current/default API-role grants; public `SECURITY DEFINER` `rls_auto_enable()` is executable by `anon` and `authenticated` | Backend + Security | Owner/purpose is confirmed and `SUPA-P0-002` is prepared. Supply the website URL/source contract; test the staged versioned remediation in isolation; configure Apple/Google providers; implement approved app schemas/policies; pass owner-positive, anonymous/other-user denial, forged-owner, expired/revoked-session, over-posted-field, abuse-control, and deletion/tombstone tests |
